...
#   SKIP_PLATFORMS manylinux2014 platforms (e.g.: cp36-cp36m) to be skipped.
...
if [[ ! -e "CMakeLists.txt" ]]; then
    (>&2 echo "Can't find or-tools sources")
    exit 1
fi
...
do
    ...
    cmake -S. -B"build_${PYTAG}" -DBUILD_DEPS=ON -DBUILD_PYTHON=ON

    cmake --build "build_${PYTAG}"
